  ...
  async function onSubmit(event: React.FormEvent) {
    ...
    try {
      if (mode === "signup") {
        ...
        setMessage(
          "Account created. Check your inbox for a confirmation from team.apsds@gmail.com (do not reply to that email), then sign in.",
        );
      } else {
    ...
    <div className="mx-auto w-full max-w-md space-y-4">
      ...
      <p className="text-center text-sm text-[var(--muted)]">
        {mode === "signin" ? (
          <>
            New here?{" "}
            <Link href="/auth/sign-up" className="underline decoration-[var(--brand)] underline-offset-4">
              sign up
            </Link>
          </>
        ) : (
          <>
            ...
            <Link href="/auth/sign-in" className="underline decoration-[var(--brand)] underline-offset-4">
              Sign in
            </Link>
          </>
        )}
